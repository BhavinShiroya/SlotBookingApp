...
function build() {
  ...
  var mainBuild = gulp.src(conf.path.tmp('/index.html'))
  ...
  var copyJsFiles = gulp.src(conf.path.tmp('assets/**/*.js'))
    .pipe(gulp.dest(conf.path.dist('assets/')));

  var copyCssFiles = gulp.src(conf.path.tmp('assets/**/*.css'))
    .pipe(gulp.dest(conf.path.dist('assets/')));
  ...
  return merge(mainBuild, copyJsFiles, copyCssFiles, font);
}
